...
- 운영 및 리팩토링
  ...
  - 주요성과
    - [테스트 커버리지 82% 달성](#atdd-와-rest-docs-를-통한-문서화)
    - [서버비용 85% 절약](#db-스키마-재설계-및-쿼리-튜닝)
    - [인증시간 93% 단축](#세션-기반-인증을-jwt-기반-인증으로-리팩토링)
    - [크래시율 99% 감소](#aws-클라우드로-인프라를-이전)
- [연도별 구현기능](#연도별-구현기능)
  - 2025년: 상점, 시즌별 아이템
...
- 설명
  ...
  - 버전 업그레이드 후 작성을 해둔 86개의 테스트 코드가 통과되지 않았습니다.
  - **테스트 커버리지가 82%** 였기에, 테스트를 모두 통과하면 실서비스 장애도 없을거라는 자신이 있었습니다.
...
- 기간: 2023.09 - 2023.12
- 사용기술: Spring Batch, JPA BatchStatement, Entity Manager
- 성과: 마이그레이션 진행을 위한 **점검시간을 1시간에서 10분** 내외로 줄임
- 설명
  - 마이그레이션 로직이 복잡했기에, 테스트 코드를 통해 검증할 필요를 느꼈습니다.
  - 엔티티 단위의 TDD 를 통해 서비스 동작에 필요한 최소 데이터를 검증하였습니다.
  - TDD 로 마이그레이션 로직을 작성하기 위해, SQL Procedure 가 아닌 JPA 로 로직을 구현했습니다.
  - JPA 로 대규모 INSERT 를 하기 위해, Entiy Manager 를 커스텀했습니다.
    - Entity Manager 는 엔티티를 save 시 INSERT 해야하는지 UPDATE 하는지 확인합니다.
...
- 성과: **서버비용 85% 절약**
- 설명
  - 재귀쿼리 혹은 N+1 쿼리를 사용하고 경우를, JOIN 쿼리로 해결할 수 있도록 스키마를 재설계했습니다.
  ...
  - 리팩토링과 최적화 덕분에 필요한 서버 리소스가 일 **51.2달러에서 7.3달러** 로 줄었습니다.
...
- 설명
  ...
  - ATDD Cycle 을 통해 **테스트 커버리지 82%** 를 달성했습니다.
...
- 설명
  ...
  - production 서버는 ECS 를 이용한 Rolling 배포 방식을 취했습니다.
    - CodePipeline 을 통해 빌드 후 컨테이너를 만들어서, ECS 에게 전달했습니다.
