...
- [연도별 구현기능](#연도별-구현기능)
  ...
  - 2023년: Feed 커뮤니티, Follower, 인기도, 알림
  - 2022년: 데이터 자동싱크, 목표 실행계획 모델링, 목표 실행계획 공유
  - 2021년: 목표와 실행계획을 적는 메모장
- 운영 및 리팩토링
  - Batch, 로깅 시스템, 데이터 마이그레이션, 쿼리튜닝, ATDD Cycle, CI/CD, AWS 인프라 구축
  - 주요성과
    - [서버비용 82% 절약](#db-스키마-재설계-및-쿼리-튜닝)
    - [테스트 커버리지 86% 달성](#프레임워크-업그레이드)
    - [인증시간 93% 단축](#세션-기반-인증을-jwt-기반-인증으로-리팩토링)
    - [안정성 99% 개선](#aws-클라우드로-인프라를-이전)
...
- 성과: 마이그레이션 진행을 위한 **점검시간을 1시간에서 10분** 내외로 단축
- 설명
  - 마이그레이션 로직이 복잡했기에, 테스트 코드를 통해 검증할 필요를 느꼈습니다.
  - 엔티티의 단위 테스트를 통해 서비스 동작에 필요한 최소 데이터를 검증하였습니다.
  - TDD 로 마이그레이션 로직을 작성하기 위해, SQL Procedure 가 아닌 JPA 로 로직을 구현했습니다.
  - JPA 로 대규모 INSERT 를 하기 위해, EntityManager 를 커스텀했습니다.
  ...
  - 최소 데이터 검증과 EnitityManager 커스텀을 통해 1시간 넘게 걸리던 마이그레이션 작업이 10분 내외로 줄었습니다.
